...
Cell 7 of the Kaggle runner lets you set `COMPARE_MODELS`, `COMPARE_SPLIT`, `COMPARE_METRICS`, `SPLIT_CHART_METRIC`, and `SPEED_CHART_METRIC` before drawing comparisons. For example, use `COMPARE_MODELS = ('dcnn_002', 'tsai_001')`, `COMPARE_SPLIT = 'validation'`, and `COMPARE_METRICS = ('accuracy', 'f1_macro')` to compare those two models on validation accuracy and F1. The cell displays a selected-score table, a training benchmark table, and three charts (selected metrics, one metric across train/validation/test, and one metric versus mean epoch time). It saves full and selected CSVs, PNGs, and `comparison_config.json` under `/kaggle/working/results/comparison_<timestamp>/`. To inspect existing result artifacts without retraining, run only Cells 1, 2, and 7.
